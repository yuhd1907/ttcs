...
interface SoftSkillsModalProps {
  isOpen: boolean;
  onClose: () => void;
  initialItems?: SoftSkillItem[];
  onSave?: (groupName: string, items: SoftSkillItem[]) => void;
}
...
export const SoftSkillsModal: React.FC<SoftSkillsModalProps> = ({
  ...
  const [groupName, setGroupName] = useState("Kỹ năng mềm");
  const [items, setItems] = useState<SoftSkillItem[]>(initialItems);
  const [selectedSkill, setSelectedSkill] = useState("");

  useEffect(() => {
    if (isOpen) {
      setGroupName("Kỹ năng mềm");
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-[16px] shadow-2xl w-full max-w-[800px] mx-4 overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#F0F0F0]">
          <h3 className="text-[18px] font-[700] text-[#121212]">Kỹ năng mềm</h3>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="px-6 py-5 max-h-[75vh] overflow-y-auto">
          ...
          <div className="flex items-start gap-1.5 mb-5">
            ...
            <p className="text-[14px] text-[#444] leading-relaxed">
              <span className="font-[600] text-[#FF9800]">Tips:</span> Liệt kê những kỹ năng mềm giúp bạn tạo thêm giá trị cho công việc bên cạnh kỹ năng chuyên môn.
            </p>
          </div>
