...
interface HardSkillsModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const HardSkillsModal: React.FC<HardSkillsModalProps> = ({
  isOpen,
  onClose,
  initialItems = [],
  onSave,
  ...
  const handleAdd = () => {
    if (!selectedSkill || !selectedExp) return;
    ...
    setItems((prev) => [
      ...prev,
      { skill: selectedSkill, experience: selectedExp },
    ]);
    setSelectedSkill("");
    setSelectedExp("");
  };
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-[16px] shadow-2xl w-full max-w-[800px] mx-4 overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#F0F0F0]">
          <h3 className="text-[18px] font-[700] text-[#121212]">Kỹ năng chính</h3>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="px-6 py-5 max-h-[75vh] overflow-y-auto">
          ...
          <div className="flex items-start gap-1.5 mb-5">
            ...
            <p className="text-[14px] text-[#444] leading-relaxed">
              <span className="font-[600] text-[#FF9800]">Tips:</span> Việc tạo kỹ năng chuyên môn theo nhóm giúp nhà tuyển dụng nhanh chóng hiểu được năng lực chuyên môn của bạn.
            </p>
          </div>
          ...
          <div className="mb-6">
            <p className="text-[14px] font-[600] text-[#121212] mb-2">
              Tên nhóm <span className="text-[#0D8EFF]">*</span>
            </p>
            <input
              type="text"
              value={groupName}
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mb-5">
              ...
              <button
                type="button"
                onClick={handleAdd}
                disabled={!selectedSkill || !selectedExp || items.length >= MAX_SKILLS}
                className={`w-[46px] h-[46px] flex-shrink-0 flex items-center justify-center rounded-[8px] text-white transition-all ${
                  (!selectedSkill || !selectedExp || items.length >= MAX_SKILLS)
                    ? "bg-[#E6F3FF] text-[#80BFFF] cursor-not-allowed"
                    : "bg-[#0D8EFF] hover:bg-[#0076E5] cursor-pointer"
                }`}
                style={(!selectedSkill || !selectedExp || items.length >= MAX_SKILLS) ? {backgroundColor: '#B3D9FF'} : {}}
              >
                ...
                {items.map((item, index) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="w-5 h-5 flex items-center justify-center text-[#999] hover:text-[#0D8EFF] hover:bg-[#E6F3FF] rounded-full transition-colors ml-1"
                    >
                      <svg width="10" height="10" viewBox="0 0 24 24" fill="none">
