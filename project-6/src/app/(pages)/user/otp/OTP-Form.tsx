...
const OTPForm = () => {
  ...
  const onSubmitSuccess = (data: OTPFormValues) => {
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/user/otp`, {
      method: "POST",
      headers: {
 ...
 ${
   errors.otp ? "border-red-500 ]" : "border-[#DEDEDE] focus:border-[#0088FF]"
        ...
        <div>
          <button
            className="h-[48px] w-[100%] bg-[#0088FF] rounded-[4px]
 text-white text-[16px] font-[700] cursor-pointer"
          >
            Xác thực
          </button>
        </div>
        <div className={"text-center"}>
          ...
          <Link href={"/user/login"} className={"text-[#5A8CFF] underline"}>
            Đăng nhập
          </Link>
        </div>
      </form>
