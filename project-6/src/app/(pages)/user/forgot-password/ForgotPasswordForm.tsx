...
const ForgotPasswordForm = () => {
  ...
  const onSubmit = (formData: ForgotPasswordFormValues) => {
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/user/forgot-password`, {
      method: "POST",
      headers: {
  ...
  return (
    <>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className={'grid grid-cols-1 gap-[15px] mt-[20px]'}>
        <div>
          <label
            htmlFor={"email"}
          ...
          <input
            ...
            className={`mt-[5px] w-[100%] h-[46px] border rounded-[4px] px-[20px] outline-none
                    ...
                    ${errors.email
              ? "border-red-500 focus:shadow-[0_0_0_4px_#F6CCD0]"
        ...
        <div className={'text-center'}>
          ...
          <Link href={'/user/login'}
                className={'text-[#5A8CFF] underline'}>
            Đăng nhập
          </Link>
        </div>
      </form>
